"""
Sitewide donate injection: add <script src="{p}js/donate.js"></script> before </body>
on every page so the Give / Donner button (floating FAB + nav CTA + footer link) renders
everywhere. donate.js does the actual DOM injection; this sweep only ensures the include
is present, handling the ../ prefix for subpages. Idempotent.
...
for f in sorted(files):
    ...
    if "js/donate.js" in s:
        skipped.append((base, "already has donate.js")); continue
    if "</body>" not in s:
        manual.append((base, "no </body> tag")); continue
    p = "" if os.path.dirname(f) == ROOT else "../"
    tag = '<script src="%sjs/donate.js"></script>\n</body>' % p
...
print("=== DONATE SWEEP (%s) ===" % ("APPLIED" if APPLY else "DRY RUN"))
